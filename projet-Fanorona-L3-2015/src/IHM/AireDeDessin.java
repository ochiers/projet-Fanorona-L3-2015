...
public class AireDeDessin extends JComponent {
	...
	public AireDeDessin(Fenetre f) {
		...
		this.plateau = new ImageIcon("Ressources" + File.separator + "images" + File.separator + "Fano9x5.jpg");
	...
	public void cheminCombo(Graphics2D drawable) {
		...
		// pointCour = new Point((int) (CoordonneesPlateau[0] * etir +
		// pCourant.colonne * segment - tailleJeton / 4 + originePlateauX),
		// (int) (CoordonneesPlateau[1] * etir + pCourant.ligne * segment -
		// tailleJeton / 4 + originePlateauY));
		drawable.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.8f));
		// drawable.drawLine(pointPrec.x + (tailleJeton / 4), pointPrec.y +
		// (tailleJeton / 4), pointCour.x + (tailleJeton / 4), pointCour.y +
		// (tailleJeton / 4));
